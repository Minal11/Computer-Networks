...
/* default port number for the seeder */
#define SEEDER_PORT 6788
...
typedef struct {
  char url[FILE_NAME_MAX]; //url tracker
...
typedef struct {
  ...
  peer_t * peers[MAX_CONNECTIONS]; // array of peer_t pointers(leechers)
  int peerCount; //Holds the value of current number of peers
//  peer_t *seeder; //seeder
  char *seedIp;
...
//  char *bindingAddress; //Ip address for binding for all incoming connections (in seeder mode)
  int mode;
  /* set once torrent is parsed */
  bt_info_t * bt_info; //the parsed info for this torrent
  int b; //b option for binding
	int lmode; //log file mode
	int smode; //save file mode
} bt_args_t;
...
typedef struct{
  int index; //which piece index
  int begin; //offset within piece
  char piece[1024]; //pointer to start of the data for a piece
	int length;
} bt_piece_t;
...
void createInterest(bt_msg_t *msg);
